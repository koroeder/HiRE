MODULE MD_SIMULATION
   CONTAINS
      ...
      SUBROUTINE TAKE_MDSTEP(CURRSTEP,CURRTEMP)
         ...
         USE MD_CALCS, ONLY: CURRENT_T, REMOVE_LINMOM, REMOVE_ANGVEL, REMOVE_COM_MOTIONS, CHECKDISTANCE
         IMPLICIT NONE
         ...
         IF (MOD(CURRSTEP,500).EQ.0) THEN
            !CALL REMOVE_ANGVEL(COORDS,VEL)
            CALL REMOVE_COM_MOTIONS(COORDS,VEL)
         END IF

         IF (USEREFLCTSPHERE) THEN
            CALL CHECKDISTANCE(COORDS,VEL)
         END IF
