MODULE MINIMISATION
   ...
   !Saving coordinates
   INTEGER :: DUMPINTMIN = 250
   INTEGER :: MINUNIT
   INTEGER :: EDECOMPUNIT
   LOGICAL :: DUMPMINCOORDST = .FALSE.
   ...
   CONTAINS
      ...
      SUBROUTINE MINIMISE(NOPT,X,ENERGY,ITDONE,MFLAG,OUTUNIT)
         ...
         IF (DUMPMINCOORDST) THEN
            CALL FILE_OPEN("min_coords.xyz", MINUNIT, .TRUE.)
            CALL FILE_OPEN("min_Edecomp.dat", EDECOMPUNIT, .TRUE.)
         END IF
         CALL MINLBFGS(NOPT, MUPDATE, X, MFLAG, ENERGY, ITDONE, OUTUNIT) 
         CALL DEALLOC_LBFGS()
         IF (DUMPMINCOORDST) CLOSE(MINUNIT)
         IF (DUMPMINCOORDST) CLOSE(EDECOMPUNIT)
