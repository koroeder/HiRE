      ...
      SUBROUTINE PASS_DATA_FOR_EXCHANGE_T(MEINITIATET,EXCHANGEDT)
         ...
         IF (SWITCHT) THEN
            ...
            IF (EXCHANGEUNITS) THEN
               !energy tracking
               CALL MPI_SEND(EUNIT,1,MPI_INT,OTHERREP,REMD_TAG+2,MPI_COMM_WORLD,ERR_CODE_MPI)
               CALL MPI_RECV(UNITDUMMY,1,MPI_INT,OTHERREP,REMD_TAG+2,MPI_COMM_WORLD,ERR_CODE_MPI)
               EUNIT = UNITDUMMY
               !coordinate tracking
               CALL MPI_SEND(XUNIT,1,MPI_INT,OTHERREP,REMD_TAG+3,MPI_COMM_WORLD,ERR_CODE_MPI)
               CALL MPI_RECV(UNITDUMMY,1,MPI_INT,OTHERREP,REMD_TAG+3,MPI_COMM_WORLD,ERR_CODE_MPI)
               XUNIT = UNITDUMMY
            ENDIF 
