!> @file
!> Contains MOD_INIT module which is used to initialise the HiRE library
...
MODULE MOD_INIT
   ...
   CONTAINS
      ...
      !> @brief
      !>
      !> Routine to open topology file and then call reading subroutine to parse topology.
      !>
      !> @param[in] TOPNAME - topology file name
      !> 
      !> @see READ_TOpOLOGY
      ...
      !> @brief
      !>
      !> The subroutine parses the topology line by line.\n
      !> The format for the topology file is:\n
      !> #filename \n
      !> SECTION CATNAME\n
      !> entries (fixed format)\n
      !> SECTION CATNAME\n
      !> entries (fixed format)\n
      !> ...\n
      !>
      !> We read a line - find the catname and then use an enumerator to select the correct fixed input format and read the entries into the correct arrays.
      !>
      !> @param[in] TOPUNIT - input unit the topology file is opened in
      !>
      !> @see READLINE
      ...
      !> @brief
      !>
      !> Routine opens and reads scale.dat file.
      !>
      ...
      SUBROUTINE READ_SCALE_DAT(SCALEDATNAME)
         ...
         DO I=1,SCORESIZE
            READ(SCUNIT, '(I4,F8.3,A)') IDUMMY, SCORE, DUMMY
            SCORE_RNA(I) = SCORE
         ENDDO
         CLOSE(SCUNIT)   
      ...
      !> @brief
      !>
      !> The routine looks for the file "restraints.dat".\n
      !> If the file exists the restraints are read and will be applied.\n
      !> If the file is not found, there are no restraints used.
