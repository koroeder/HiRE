...
contains
    ...
    function rot_q_multiply(q1, q2) result(q3)
        implicit none
        real(kind = real64), intent(in) :: q1(4), q2(4)
        real(kind = real64) :: q3(4)
    ...
    function rot_rotate_aa(p1, p2) result(p3)
        implicit none
        real(kind = real64), intent(in) :: p1(3), p2(3)
        real(kind = real64) :: p3(3)
    ...
    function rot_aa2mx(p) result(m)
        implicit none
        real(kind = real64), intent(in) :: p(3)
!        real(kind = real64) m(3,3), tmp(3,3)
...
!        call rmdrvt(p, m, tmp, tmp, tmp, .false.)
    ...
    function rot_mx2q(mi) result(q)
        implicit none
        real(kind = real64), intent(in) :: mi(3,3)
        real(kind = real64) :: q(4), m(3,3)
    ...
    function rot_small_random_aa(maxtheta) result(p)
        use vec3
        implicit none
        real(kind = real64), intent(in) :: maxtheta
        real(kind = real64) :: p(3)
        real(kind = real64) :: u, dprand
        real(kind = real64) :: s
    ...
    subroutine rot_takestep_aa(p, maxtheta)
        implicit none
        real(kind = real64), intent(inout) :: p(3)
        real(kind = real64), intent(in) :: maxtheta
    ...
    subroutine rot_construct_unit_system(x, e)
        use vec3
        implicit none
        real(kind = real64) x(3,3)
        real(kind = real64) e(3,3)
        real(kind = real64) tmp(3)
        integer i
    ...
    function rot_get_orientation_aa(x, xref) result(p)
        use vec3
        implicit none
        real(kind = real64), intent(in) :: x(3,3), xref(3,3)
        real(kind = real64) p(3)
        real(kind = real64) eref(3,3), e(3,3), minv(3,3)
