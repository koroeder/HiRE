...
SUBROUTINE FINALIO
  ...
  USE HIRE_OPEP_INTERFACE_MOD, ONLY: OPEP_FINISH, OPEP_WRITE_PDB, ATOM_NAMES
...
10   FORMAT('Energy of minimum ',I6,'=',G20.10,' first found at step ',I8,' after ',I20,' function calls')
     ...
     IF (MPIT) THEN
        ...
        CALL OPEP_WRITE_PDB(NATOMS, QMINP(J1,:),'lowest.'//TRIM(ADJUSTL(J1_STRING))//&
                   &'.'//TRIM(ADJUSTL(MYNODE_STRING))//'.pdb')
        ...
        CLOSE(MYUNIT3)
     ELSE
        CALL OPEP_WRITE_PDB(NATOMS, QMINP(J1,:),'lowest.'//TRIM(ADJUSTL(J1_STRING))//'.pdb')
        !also write start file to be used for OPTIM runs
        MYUNIT3=GETUNIT()
