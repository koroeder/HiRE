...
USE PREC
USE COMMONS, ONLY: NATOMS, DEBUG, GEOMDIFFTOL, TEMP, MYUNIT, NPAR
IMPLICIT NONE
REAL(KIND = REAL64)  :: NEWCOORDS(3*NATOMS), OLDCOORDS(3*NATOMS), OLDCOORDSCP(3*NATOMS)
REAL(KIND = REAL64)  :: RMAT(3,3), DISTANCE, ENEW, EOLD, GRAD(3*NATOMS), DPRAND
INTEGER           :: NULLMOVES(NPAR), JP
LOGICAL           :: ATEST 

OLDCOORDSCP(:)=OLDCOORDS(:) ! make local copy to avoid returning modified markov coordinates
!CALL ALIGN_DECIDE(NEWCOORDS,OLDCOORDSCP,NATOMS,DEBUG,BOXLX,BOXLY,BOXLZ,PERIODIC,TWOD,DISTANCE,DIST2,RIGID,RMAT)
!Using MINPERMDIST as default 
CALL MINPERMDIST(NEWCOORDS,OLDCOORDSCP,NATOMS,DEBUG,DISTANCE,RMAT)
CALL POTENTIAL(OLDCOORDSCP,GRAD,EOLD,.FALSE.,.FALSE.,.FALSE.) ! possibly
CALL POTENTIAL(NEWCOORDS,GRAD,ENEW,.FALSE.,.FALSE.,.FALSE.) ! redundant
...
USE COMMONS, ONLY: STEP,TEMP,NACCEPT,SRATIO,TRATIO,NPAR,MYUNIT,SUMTEMP,SUMSTEP,TMOVE, &
                 & GROUPROTT,GR_SCALEPROB,GR_SCALEROT, DEBUG
USE GROUPROTMOD
USE PREC
IMPLICIT NONE
REAL(KIND = REAL64)  :: P0, P1, P0T, P1T
...
IF (DEBUG) THEN ! print some useful statistics
   WRITE(MYUNIT,'(4(A,F8.5),A)')'FIXRATIO>Step ratio (mean) = ',P0,' (',P0T,'); Temperature ratio (mean) = ',P1,' (',P1T,')'
   WRITE(MYUNIT,'(A,F10.5,A,F10.5)')'FIXRATIO>Step is now ',STEP(JP),'; Temperature is now ',TEMP(JP)
   IF (JP==NPAR) WRITE(MYUNIT,'(A,F10.5,A,F10.5)')'FIXRATIO>Average step =',SUMSTEP/NPAR/NSTEPS*NACCEPT,&
                       '; Average temperature =',SUMTEMP/NPAR/NSTEPS*NACCEPT
ENDIF
...
USE COMMONS, ONLY: MYUNIT,NPAR,SUMTEMP,SUMSTEP,NACCEPT
USE PREC
IMPLICIT NONE
REAL(KIND = REAL64)  :: P0, P1
...
WRITE(MYUNIT,'(A,F10.5,A,F10.5)')'FINALRATIO>Average step =',SUMSTEP,&
