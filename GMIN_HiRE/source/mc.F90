...
#endif
      REAL(KIND = REAL64) :: DPRAND, DPRAND_UNIVERSAL
...
!  Standard canonical sampling.
      ...
      SUBROUTINE SAVE_HB(J1, JP)
        ...
        REAL(KIND = REAL64) :: HBX(3*NATOMS), HBF(3*NATOMS), HBE

        HBSAVET=(MOD(J1,HBNSTEPS).EQ.0)
        IF(HBSAVET)THEN
            HBX(:)=COORDSO(:,JP)
            CALL POTENTIAL(HBX, HBF, HBE, .FALSE., .FALSE.)
            HBSAVET=.FALSE.
        ENDIF
     ...
     SUBROUTINE ADD_SAXS_FORCE(J1,JP,ATEST,MCTEMP)
        ...
        ENDIF
        SAXSX(:)=COORDSO(:,JP)
        CALL POTENTIAL(SAXSX, SAXSF, SAXSEO, .FALSE., .FALSE.)
        SAXSX(:)=COORDS(:,JP)
        CALL POTENTIAL(SAXSX, SAXSF, SAXSE, .FALSE., .FALSE.)
        SAXSFORCET=.FALSE. 
        SAXSSAVET=.FALSE. 
