MODULE MEASURE
! Created the module to measure distances, angles and dihedral angles using a generic set of interfaces.
   USE PREC
   IMPLICIT NONE
...
CONTAINS
   ...
   FUNCTION CROSS_PRODUCT(A, B) RESULT(C)
      ...
      REAL(KIND = REAL64), DIMENSION(1:3), INTENT(IN)    :: A, B
      REAL(KIND = REAL64), DIMENSION(1:3)                :: C
   ...
   FUNCTION DOUBLE_DISTANCE(X1, Y1, Z1, X2, Y2, Z2) RESULT(DISTANCE)
      ! This one takes 6 doubles (2 sets of 3 Cartesian coordinates) 
      ! and returns the magnitude of the distance as a double.
      REAL(KIND = REAL64), INTENT(IN)  :: X1, Y1, Z1, X2, Y2, Z2
   ...
   FUNCTION ATOMS_DISTANCE(ATOM_I, ATOM_J, COORDS) RESULT(DISTANCE)
      ! This one takes calculates the distance between the atoms with 
      ! index ATOM_I and ATOM_J from COORDS and returns the magnitude as a double.
      INTEGER, INTENT(IN)                          :: ATOM_I, ATOM_J
      REAL(KIND = REAL64), DIMENSION(:), INTENT(IN)   :: COORDS
      REAL(KIND = REAL64)                             :: DISTANCE
   ...
   ! This function takes 9 doubles (3 sets of 3 Cartesian coordinates) and returns the angle 1-2-3 as a double.
      REAL(KIND = REAL64), INTENT(IN)     :: X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3
      REAL(KIND = REAL64)                 :: ANGLE
      ! This is a set of vectors
      REAL(KIND = REAL64), DIMENSION(1:3) :: D21, D23, CROSS
   ...
   FUNCTION DOUBLE_VECTOR_ANGLE(X1, Y1, Z1, X2, Y2, Z2) RESULT(ANGLE)
      ! This function takes 6 doubles (2 sets of 3 Cartesian vector directions) 
      ! and returns the angle between the two vectors as a double.
      REAL(KIND = REAL64), INTENT(IN)     :: X1, Y1, Z1, X2, Y2, Z2
      REAL(KIND = REAL64)                 :: ANGLE
      ! This is a set of vectors
      REAL(KIND = REAL64), DIMENSION(1:3) :: V1, V2, CROSS
   ...
   FUNCTION ATOMS_ANGLE(ATOM_I, ATOM_J, ATOM_K, COORDS) RESULT(ANGLE)
      ! This function calculates the angle I-J-K between the atoms with 
      ! index ATOM_I, ATOM_J and ATOM_K and returns the angle as a double.
      INTEGER, INTENT(IN)                          :: ATOM_I, ATOM_J, ATOM_K
      REAL(KIND = REAL64), DIMENSION(:), INTENT(IN)   :: COORDS
      REAL(KIND = REAL64)                             :: ANGLE
      ! This is a set of vectors
      REAL(KIND = REAL64), DIMENSION(1:3)             :: D_JI, D_JK, CROSS
   ...
   ! This function takes 12 doubles (4 sets of 3 Cartesian coordinates) and returns the dihedral angle 1-2=3-4 as a double.
      REAL(KIND = REAL64), INTENT(IN)  :: X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3, X4, Y4, Z4
      REAL(KIND = REAL64)              :: DIHEDRAL
      ! Vectors and cross products
      REAL(KIND = REAL64), DIMENSION(1:3) :: D12, D23, D34, CROSS_12_23, CROSS_23_34
      ! Args for atan2
      REAL(KIND = REAL64)                 :: FIRST, SECOND
   ...
   FUNCTION DOUBLE_VECTOR_DIHEDRAL(X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3) RESULT(DIHEDRAL)
      ! This function takes 9 doubles (3 sets of 3 Cartesian vector directions) and 
      ! returns the dihedral angle between 1 and 3 about 2 as a double.
      REAL(KIND = REAL64), INTENT(IN)     :: X1, Y1, Z1, X2, Y2, Z2, X3, Y3, Z3
      REAL(KIND = REAL64)                 :: DIHEDRAL
      ! Vectors and cross products
      REAL(KIND = REAL64), DIMENSION(1:3) :: V1, V2, V3, CROSS_12, CROSS_23
      ! Args for atan2
      REAL(KIND = REAL64)                 :: FIRST, SECOND
   ...
   FUNCTION ATOMS_DIHEDRAL(ATOM_I, ATOM_J, ATOM_K, ATOM_L, COORDS) RESULT(DIHEDRAL)
      ! This function calculates the dihedral angle I-J=K-L between the atoms with 
      ! index ATOM_I, ATOM_J, ATOM_K and ATOM_L and returns the angle as a double.
      INTEGER, INTENT(IN)                          :: ATOM_I, ATOM_J, ATOM_K, ATOM_L
      REAL(KIND = REAL64), DIMENSION(:), INTENT(IN)   :: COORDS
      REAL(KIND = REAL64)                             :: DIHEDRAL
      ! Vectors and cross products
      REAL(KIND = REAL64), DIMENSION(1:3)             :: D_IJ, D_JK, D_KL, CROSS_IJ_JK, CROSS_JK_KL
      ! Args for atan2
      REAL(KIND = REAL64)                             :: FIRST, SECOND
