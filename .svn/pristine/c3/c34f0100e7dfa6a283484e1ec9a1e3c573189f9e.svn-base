...
contains
  ...
  subroutine cross(v, v1, v2) 
    ...
    real(kind = real64), dimension(3), intent(out) :: v
  ...
  subroutine rotation_matrix(natoms,posa,posb,M)
    ...
    real(kind = real64), dimension(3*natoms), target :: posa,posb
    real(kind = real64), dimension(3,3)  :: cpp,cpp1,cpp2,cpp3    ! Matrix C"  
    real(kind = real64), dimension(3, 3) :: M,M1,M2,M3            ! Matrix M 
    real(kind = real64), dimension(3, 3) :: matrc                 ! Matrix C 
    real(kind = real64), parameter :: eps = 1.0E-12
    real(kind = real64) :: alpha, beta, gama, s, t, st2, st
...
!         write(*,*) "Exiting after ", maxiter, "iterations."
  ...
  subroutine minimize_rmsd(nat,posref,pos,delr,rmsd,npart,calpha_in,atomic_type_in)
    ...
    real(kind = real64), parameter :: THRESHOLD = 0.1          ! In Angstroems
    ...
    real(kind = real64), dimension(3*nat), intent(inout), target :: posref, pos
    real(kind = real64), dimension(:), allocatable, target :: posa, posb, pospb

    real(kind = real64), dimension(:), pointer :: xa, ya, za   ! Pointers for working position
    real(kind = real64), dimension(:), pointer :: xb, yb, zb   ! Pointers for reference position
    real(kind = real64), dimension(:), pointer :: xpb, ypb, zpb
    real(kind = real64), dimension(3,3) :: M
    real(kind = real64) :: dr2,dr,delr2, delx,dely,delz,delr, rmsd
    integer :: i,j,npart
  ...
  subroutine geometric_center(natoms,vector)
    implicit none
    integer :: natoms
    real(kind = real64), dimension(3*natoms), intent(inout), target :: vector
    real(kind = real64), dimension(3) :: centrd  ! centroid or center of mass     
    
    real(kind = real64), dimension(:), pointer :: x, y, z     ! Pointers for coordinates
  ...
  subroutine remove_total_momentum(natoms,vec,mass)
    implicit none
    integer, intent(in) :: natoms
    real(kind = real64), dimension(3*natoms), intent(inout), target :: vec
    real(kind = real64), dimension(3*natoms), intent(in),    target :: mass 
    real(kind = real64), dimension(:), pointer :: vx, vy, vz

    real(kind = real64) :: pxtot, pytot, pztot, mcm
...
!    vz = vz - pztot/mass(1:natoms)   
  ...
  subroutine center_of_mass(natoms,vec,mass)
    ...
    real(kind = real64) :: cm1,cm2,cm3,total_mass
  ...
  subroutine remove_rotation(natoms,pos,vel,mass) 
    ...
    integer, intent(in) :: natoms
    real(kind = real64), dimension(3*natoms), intent(in), target    :: pos, mass
    real(kind = real64), dimension(3*natoms), intent(inout), target :: vel
    
    integer :: i
    real(kind = real64), dimension(:), pointer :: x, y, z, vx, vy, vz
!    real(kind = real64) :: total_mass 
    real(kind = real64) :: trace
!    real(kind = real64), dimension(3)   :: cm
    real(kind = real64), dimension(3)   :: l, temp, v, l1, v1, o
    real(kind = real64), dimension(3,3) :: inertia, i1
...
!    cm = cm/total_mass
   ...
   subroutine solve_3x3(U,V,W)
    ...
    real(kind = real64), dimension(3), intent(in)   :: V
    real(kind = real64), dimension(3), intent(out)  :: W

    real(kind = real64) :: a, b, c, d, e, f, p, q, o
    real(kind = real64) :: af_de, aq_eo, ab_dd, ac_ee
  ...
  subroutine matrix_c(natoms,posa,posb,mtr_c)
    ...
    integer, intent(in) :: natoms
    real(kind = real64), dimension(3*natoms), intent(in), target :: posa, posb
    
    real(kind = real64), dimension(3,3), intent(out) :: mtr_c
    real(kind = real64), dimension(:), pointer :: xa, ya, za, xb, yb, zb
    integer :: i
  ...
  subroutine get_momentum_rotation(nstep,natoms,pos,vel,mass)
    ...
    real(kind = real64), dimension(3*natoms), intent(in), target :: pos, vel, mass
    real(kind = real64), dimension(:), pointer :: x, y, z, vx, vy, vz, ms

    real(kind = real64), dimension(3) :: com, momentum
    real(kind = real64) :: total_mass
    real(kind = real64) :: trace
    real(kind = real64), dimension(3) :: temp, L, V, L1, O
    real(kind = real64), dimension(3,3) :: inertia, i1
