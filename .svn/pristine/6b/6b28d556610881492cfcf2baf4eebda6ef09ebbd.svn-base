module ion_pair
  ...
  TYPE rdf_t
     real(kind = real64),dimension(200)                :: rdf_sph,v_sph,mix_1,mix_2
     real(kind = real64)                               :: dens
  END TYPE rdf_t

  TYPE ip_potential_t
     real(kind = real64)                               :: rmin,rmax,dr,box_ref,rc
     real(kind = real64),DIMENSION(mxgrid,mxip_type,2) :: v_grid,dv_grid
     real(kind = real64),DIMENSION(mxip_type,2)        :: alfa,beta
  ...
  real(kind = real64)                      :: temp_ip=0.59227! here set a value for ambient temp.
  real(kind = real64),DIMENSION(3)         :: energy_ip
  real(kind = real64)                      :: ion_pair_scaling
...
!            IF(resindex_ip_plus(i_cp).eq.(resindex_ip_minus(i_cn)-3)) cycle
  ...
  subroutine ip_potential_init
    ...
    real(kind = real64)                    :: rmin,rmax,dr,box,r
...
!--- Open I/O
      ...
      open(unit=nf,file='ip_potential.dat',status='old')
...
!--- local variables  
    ...
    integer                        :: nrdf=38
    real(kind = real64)            :: pi,v,r1,r2,dr,box
...
!-------------------------------------------------------------------------------
  ...
  subroutine ion_pair_force(x,f,e_ip)
      ...
      use PBC_defs, only: pbc_mic,periodicBC
...
!----- local variables 
      ...
      integer i,I1,I2,IT
      integer ica1,ica2
      real(kind = real64) x1,x2,y1,y2,z1,z2,d,d2,x12,y12,z12
      real(kind = real64) fx,fy,fz,e,rc
      real(kind = real64) alfa,beta,cteta,inv_mix,mix
      real(kind = real64) xca1,yca1,zca1,xca2,yca2,zca2
      real(kind = real64) nx1,ny1,nz1,nx2,ny2,nz2,nn1,nn2
...
!-------------------------------------------------------------------------------
    ...
    subroutine ip_potential_grid(x12,y12,z12,r,it,fx,fy,fz,e,i_state)
      ...
      integer it
      real(kind = real64) x12,y12,z12
      real(kind = real64) fx,fy,fz,e,r
      real(kind = real64) rmin
      
!---- Function 
  
      real(kind = real64) interpolation

!---- Local Variables ------------------

      integer ir,i_state
      real(kind = real64) f1
      real(kind = real64) factor,dr,ra,dva,dvb,va,vb
...
!-------------------------------------------------------------------------------
  ...
  function interpolation(dx,xa,x,fxa,fxb)
    ...
    real(kind = real64),INTENT(IN)         :: fxa,fxb,dx,xa,x
